...
// 3 make sure to select buttons when they are avaliable
...
//console.log(filterBtns); no access so move to join in the e-listener
...
function displayMenuItems(menuItems) {
  ...
  displayMenu = displayMenu.join('')//join return the array as a string
  sectionCenter.innerHTML = displayMenu
}
function displayMenuButtons() {
  // 1 using reduce to return unique category
  ...
  const filterBtns = document.querySelectorAll('.filter-btn')
  //filter items
  filterBtns.forEach((btn) => { //função dos btn vai gerar função evento
    btn.addEventListener('click', (e) => {
      //console.log(e.currentTarget.dataset);
      //dataset é categoria da .fiter-btn - data-id
