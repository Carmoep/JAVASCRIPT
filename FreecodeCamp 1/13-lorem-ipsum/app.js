...
const text = [
  ...
  const textEl = document.querySelector('.lorem-text')

  formEl.addEventListener('submit', (e) => {
    e.preventDefault() // sem botão
    //console.log('oi');
    //const value = amount.value - string
    const value = parseInt(amount.value) //number or NAN(no input)
    const random = Math.floor(Math.random() * text.length)
    //console.log(value);
    //empty
    //-1
    // > 9
    //console.log(isNaN(value)); - true or false
    if(isNaN(value) || value <= 0 || value > 9) {
        textEl.innerHTML = `<p class="result">${text[0]}</p>`
    } else {
